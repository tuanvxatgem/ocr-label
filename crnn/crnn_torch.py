...
from config import ocrModel, LSTM_FLAG, chinesesModel, GPU
...
def crnnSource():
    ...
    if torch.cuda.is_available() and GPU:
        model = CRNN(32, 1, len(alphabet) + 1, 256, 1, lstmFlag=LSTM_FLAG).cuda()  ##LSTM_FLAG=True crnn 否则 dense ocr
    else:
        model = CRNN(32, 1, len(alphabet) + 1, 256, 1, lstmFlag=LSTM_FLAG).cpu()
